...
namespace GraphTutorial.Graph
{
    public class GraphHelper
    {
        ...
        public static async Task<IEnumerable<Event>> GetCurrentWeekCalendarViewAsync(DateTime today, string timeZone)
        {
            ...
            try
            {
                var events = await graphClient.Me
                    .CalendarView
                    .Request(viewOptions)
                    //Send user time zone in request so date/time in response will be in preferred time zone
        ...
        private static DateTime GetUtcStartOfWeekInTimeZone(DateTime today, string timeZoneId)
        {
            ...
            //.NET Core's FindSystemTimeZoneById needs IANAon Linux/MacOS,
        ...
        public static async Task CreateEvent(
            ...
            string body = null)
        {
            //Create a new Event object with required values
            ...
            //Only add attendees if there are actual values
